...
df.ves0.55_withTimeAnalysis<-subset(df.ves0.55_withTimeAnalysis, t>=t.start & t<=t.end.0.55)
df.ves0.1_withTimeAnalysis<-subset(df.ves0.1_withTimeAnalysis, t>=t.start & t<=t.end.0.1)
...
plot.VE.RR.ves0.55_withTime<-ggplot(df.ves0.55_withTimeAnalysis, aes(x=t, y=VE_symptRR_timeDep.50.)) +
  geom_hline(yintercept=ve.susc.0.55, linetype="dashed", 
             color = "grey40", linewidth=2) +
  theme_classic() +
  geom_ribbon(aes(ymin=VE_symptRR_timeDep.25., ymax=VE_symptRR_timeDep.75., fill=Label_testingDiff, 
                  color=Label_testingDiff), alpha=0.3, linewidth=ribbon.line.width) +
  scale_x_continuous(limits = c(t.start, t.end.0.55),  breaks = seq(t.start,t.end.0.55,10)) + 
  scale_y_continuous(limits = c(-2.5, 1), breaks= seq(-2.5, 1, 0.5)) +
...
#Select every 25th Timestep to give an overview of VE Estimates across the epidemic
table.data.veS.0.55<-subset(df.ves0.55_withTimeAnalysis, t %in% seq(25,150,25))
table.data.veS.0.1<-subset(df.ves0.1_withTimeAnalysis, t %in% seq(25,100,25))

#Reorder Testing Difference Factor
table.data.veS.0.55$Label_testingDiff<- factor(table.data.veS.0.55$Label_testingDiff, 
                                               levels = c("equal", "modUnequal", "highUnequal"))
table.data.veS.0.1$Label_testingDiff<- factor(table.data.veS.0.1$Label_testingDiff, 
                                              levels = c("equal", "modUnequal", "highUnequal"))
...
VE.RR_statsTable_function<-function(table.df,VE.S){
  ...
  table_data <- data.frame(
    ...
    VE_WithoutTime = paste0(
      sprintf("%.3f", table.df$VE_symptRR_observed.50.), " (",
      sprintf("%.3f", table.df$VE_symptRR_observed.5.), " - ",
      sprintf("%.3f", table.df$VE_symptRR_observed.95.), ")"
    ),
    
    VE_WithTime = paste0(
      sprintf("%.3f", table.df$VE_symptRR_timeDep.50.), " (",
      sprintf("%.3f", table.df$VE_symptRR_timeDep.5.), " - ",
      sprintf("%.3f", table.df$VE_symptRR_timeDep.95.), ")"
    ),
    
    VacStatusCoef_WithTime = paste0(
      sprintf("%.3f", table.df$VacStatus_coef_timeDep_VeRR.50.), " (",
      sprintf("%.3f", table.df$VacStatus_coef_timeDep_VeRR.5.), " - ",
      sprintf("%.3f", table.df$VacStatus_coef_timeDep_VeRR.95.), ")"
    ),
    
    stringsAsFactors = FALSE
  )
  
  # Arrange the data (grouping by testing group, then time)
  table_data <- table_data %>% arrange(TestingDiff, t)
  
  # Build the table and add a top header row spanning all columns
  final.table <- table_data %>%
    kable(format = "html") %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                  full_width = FALSE) %>%
    collapse_rows(columns = 1, valign = "top") %>%
    add_header_above(setNames(ncol(table_data), paste0("Cohort Design; VE.S = ", VE.S)))
  
  return(final.table)
}

VE.RR_statsTable_function(table.df=table.data.veS.0.55, VE.S=0.55)
VE.RR_statsTable_function(table.df=table.data.veS.0.1, VE.S=0.1)
...
plot.VE.OR.ves0.55_withTime<-ggplot(df.ves0.55_withTimeAnalysis, aes(x=t, y=VE_symptOR_timeDep.50.)) +
  geom_hline(yintercept=ve.susc.0.55, linetype="dashed", 
             color = "grey40", linewidth=2) +
  theme_classic() +
  geom_ribbon(aes(ymin=VE_symptOR_timeDep.25., ymax=VE_symptOR_timeDep.75., fill=Label_testingDiff, 
                  color=Label_testingDiff), alpha=0.3, linewidth=ribbon.line.width) +
  scale_x_continuous(limits = c(t.start, t.end.0.55),  breaks = seq(t.start,t.end.0.55,10)) + 
  scale_y_continuous(limits = c(-2.5, 1), breaks= seq(-2.5, 1, 0.5)) +
...
VE.OR_statsTable_function<-function(table.df, VE.S){
  ...
  table_data <- data.frame(
    ...
    VE_WithoutTime = paste0(
      sprintf("%.3f", table.df$VE_symptOR_observed.50.), " (",
      sprintf("%.3f", table.df$VE_symptOR_observed.5.), " - ",
      sprintf("%.3f", table.df$VE_symptOR_observed.95.), ")"
    ),
    
    VE_WithTime = paste0(
      sprintf("%.3f", table.df$VE_symptOR_timeDep.50.), " (",
      sprintf("%.3f", table.df$VE_symptOR_timeDep.5.), " - ",
      sprintf("%.3f", table.df$VE_symptOR_timeDep.95.), ")"
    ),
    
    Intercept_WithTime = paste0(
      sprintf("%.3f", table.df$Intercept_coef_timeDep_VeOR.50.), " (",
      sprintf("%.3f", table.df$Intercept_coef_timeDep_VeOR.5.), " - ",
      sprintf("%.3f", table.df$Intercept_coef_timeDep_VeOR.95.), ")"
    ),
    
    VacStatus_WithTime = paste0(
      sprintf("%.3f", table.df$VacStatus_coef_timeDep_VeOR.50.), " (",
      sprintf("%.3f", table.df$VacStatus_coef_timeDep_VeOR.5.), " - ",
      sprintf("%.3f", table.df$VacStatus_coef_timeDep_VeOR.95.), ")"
    ),
    
    Time_WithTime = paste0(
      sprintf("%.3f", table.df$TimeStep_coef_timeDep_VeOR.50.), " (",
      sprintf("%.3f", table.df$TimeStep_coef_timeDep_VeOR.5.), " - ",
      sprintf("%.3f", table.df$TimeStep_coef_timeDep_VeOR.95.), ")"
    ),
  ...
  final.table <- table_data %>%
    ...
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                  full_width = FALSE) %>%
    collapse_rows(columns = 1, valign = "top") %>%
    add_header_above(setNames(ncol(table_data), paste0("Test-negative Design; VE.S = ", VE.S)))
  
  return(final.table)
}

VE.OR_statsTable_function(table.df=table.data.veS.0.55, VE.S=0.55)
VE.OR_statsTable_function(table.df=table.data.veS.0.1, VE.S=0.1)
...
#Save Figure 17 (warnings are just because we are only plotting part of the time we have recorded)
...
#Save Figure 18 (warnings are just because we are only plotting part of the time we have recorded)
...
pdf(file=paste(appfig21.pathway,"AppFig.21b_VE.RR.0.55.Equal.Testing.pdf",sep="/"), 
    width=7.5, height=6)
VE.RR.ves0.55.equal.testing
dev.off()

pdf(file=paste(appfig21.pathway,"AppFig.21c_VE.OR.0.55.Target.pdf",sep="/"), 
    width=7.5, height=6)
