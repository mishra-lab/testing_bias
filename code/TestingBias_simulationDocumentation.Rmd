---
title: "Testing Bias Simulation Documentation"
date: "2025-04-23"
...
The project involves eight R scripts to run the ABM model, generate the datasets, conduct analyses and produce figures.
...
The following three R scripts contain the code to run the agent-based model (ABM) and create the simulated data for the sensitivity analyses that accounts for time when estimating symptomatic VE:
...
5. ***AppendixAnalyses_and_Figures.R***: contains the code to conduct the supporting analyses found in the appendix and to generate Appendix Figures.

### Datasets

There are two main types of datasets produced by ***ABM_Main.R***:
...
2. *agg_df* or *df.ve.susc* (with the exception of those ending with "withTimeAnalysis"): contain population-level information across timesteps and across three testing scenarios (originally represented as three separate data
frames: *aggDF_equal*, *aggDF_modUnequal*, and *aggDF_highUnequal* that were combined). These datasets are saved and are used to generate most results and figures (see below). 

There is an additional type of appendix dataset produced by ***ABM_App_withTime.R***:

1. *df.ve.susc0.55_withTimeAnalysis* or *df.ve.susc0.1_withTimeAnalysis*: contain population-level information including analyses from statistical models that account for time across timesteps and across three testing scenarios (originally represented as three separate dataframes: *aggDF_equal*, *aggDF_modUnequal*, and *aggDF_highUnequal* that were combined)
...
## Appendix ABM, Aggdf and IndTestdf With Time Details 

### Description

***ABM_App_withTime.R***, ***Aggdf_appFunctions_withTime.R*** and ***IndTestdf_appFunctions.R*** are used to conduct additional sensitivity analyses that account for time when estimating symptomatic VE. ***ABM_App_withTime.R*** is almost identical to ***ABM_Main.R*** but calls on the additional R files to generate individual-level data and run statistical analyses that account for time.

### Main Functions

These functions are all found in ***Aggdf_appFunctions_withTime.R***. Below is a short description for each function.


##### **Function Name:** <span style="color: purple"> _aggdf_Create{}_</span>

**Short Description:** Calculates the true and sampled numbers and proportions of those infected with symptomatic SARS-CoV-2 and who are symptomatic with alternate etiologies. Also calculates symptomatic vaccine effectiveness estimates derivied using the retrospective cohort design and the retrospective test-negative design.

##### **Function Name:** <span style="color: purple"> _aggdf_Stats{}_</span>

**Short Description:** Calls on a helper function to aggregate information on symptomatic individuals who have tested by vaccination status (<span style="color: purple"> _agg.df_addSampleSize_function{}_</span>), runs the statistical analyses that accounts for time (Cox model for the retrospective cohort; Time-adjusted logistic model for the test-negative design) on the appropriately sampled data (using <span style="color: purple"> _testSampling_function{}_</span>) and calls on a helper function (<span style="color: purple"> _agg.df_addTimeDepCoefs_function{}_</span>) to add statistical model information to an existing dataframe.

##### **Function Name:** <span style="color: purple"> _agg.df_addSampleSize_function{}_</span>

**Short Description:** Helper function that calculates the numbers of tested symptomatic individuals who are infected with SARS-CoV-2 or are symptomatic due to alternate etiologies by vaccination status.

##### **Function Name:** <span style="color: purple"> _agg.df_addTimeDepCoefs_function{}_</span>

**Short Description:** Helper function that pulls the coefficients, standard errors and p-values from statistical models and adds them to an existing dataframe.

##### **Function Name:** <span style="color: purple"> _testSampling_function{}_</span>

**Short Description:** Helper function that creates a sampled dataset for a study conducted at a specific timepoint. The sampling procedure includes selecting either the first positive SARS-CoV-2 test for individuals who have at least one positive SARS-CoV-2 test or a random negative test for individuals who only have negative tests.
...
### _agg_df and df.ve.susc Datasets_ 

*Reminder: These are the datasets found in the "data" folder in the github repository. _df.ve.susc_ ending with "withTimeAnalysis" are different datasets; A variable list for these datasets can be found in the next section* 
...
-   **FalseNeg_withTestSens_unvac**<-Cumulative number of *unvaccinated*
    individuals who have been recorded as having a negative SARS-CoV-2 test due to imperfect test sensitivity. This variable is calculated in "Appendix Analyses_and_Figures.R"
...
-  **VE_symptRR_degUnderNeg_testSens**\<-measure of the extent to which observed vaccine effectiveness
    against symptomatic SARS-CoV-2 infection (calculated using the relative risk) underestimates true symptomatic vaccine effectiveness when also varying test sensitivity.This variable is calculated in "Appendix Analyses_and_Figures.R"
    
-  **VE_symptOR_degUnderNeg_testSens**\<-measure of the extent to which observed vaccine effectiveness
    against symptomatic SARS-CoV-2 infection (calculated using the odds ratio) underestimates true symptomatic vaccine effectiveness when also varying test sensitivity. This variable is calculated in "Appendix Analyses_and_Figures.R"
...
-   **Label\_ epiPhase** \<- a string that has two possible labels,
    "Highest Growth", and "Peak" indicating the phase of the epidemic that estimates are taken.This variable is calculated in "Appendix Analyses_and_Figures.R"

-   **t** \<- the timestep that a study has been conducted

-   **Test_sens** \<- the level of test sensitivity.This variable is calculated in "Appendix Analyses_and_Figures.R"
...
-   **totalNum_sims**\<- the total number of epidemic realizations used in that scenario
...
### _df.ve.susc0.55_withTimeAnalysis and df.ve.susc0.1_withTimeAnalysis Datasets_ 


#### Column names (Variable Names)

-   **Intercept_coef_timeDep_VeOR**\<- Intercept from a time-adjusted logistic regression model for estimating symptomatic vaccine effectiveness using the test-negative design.

-   **TimeStep_coef_timeDep_VeOR**\<-Coefficient for time since the start of a study from a time-adjusted logistic regression model for estimating symptomatic vaccine effectiveness using the test-negative design.

-   **VacStatus_coef_timeDep_VeOR**\<-Coefficient for vaccination status from a time-adjusted logistic regression model for estimating symptomatic vaccine effectiveness using the test-negative design.
...
-   **VE_symptOR_timeDep**\<-Estimate of vaccine effectiveness against symptomatic COVID-19 infection based on a time-adjusted logistic regression model using a test-negative design. Here vaccine effectiveness is defined as VE$_{logit}$ = (1 - odds ratio$_{logit}$ *[OR$_{logit}$]*) where *OR$_{logit}=$* $exp$(**VacStatus_coef_timeDep_VeOR**).

-   **VE_symptRR_timeDep**\<-Estimate of vaccine effectiveness against symptomatic COVID-19 infection based on a Cox model using a cohort design. Here vaccine effectiveness is defined as VE$_{HR}$= (1 - hazard ratio *[HR]*) where *HR=* $exp$(**VacStatus_coef_timeDep_VeRR**).

 The following variables are also included in these datasets and are defined above: **Label_testingDiff**,	**t**, **t_highChange**, **VE_symptOR_observed** and **VE_symptRR_observed**.
